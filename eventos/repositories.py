...
class EquipoRepository:
    ...
    def get_by_nombre(nombre: str) -> Optional[Equipo]:
        """Obtener un equipo por su nombre"""
        try:
            return Equipo.objects.get(nombre=nombre)
        except Equipo.DoesNotExist:
            return None

    @staticmethod
    def get_by_deporte(deporte_id: int) -> QuerySet[Equipo]:
        """Obtener equipos filtrados por deporte"""
        return Equipo.objects.filter(deporte_id=deporte_id).select_related('deporte')

    @staticmethod
    def create(nombre: str, deporte_id: int, ciudad: str, fecha_fundacion=None) -> Equipo:
    ...
    @staticmethod
    def update(equipo: Equipo, nombre: str = None, deporte_id: int = None,
